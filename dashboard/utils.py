...
def export_to_pdf(events, user):
    ...
    p.drawString(50, y, "Events Summary:")
    y -= 20
    
    p.setFont("Helvetica", 9)
    for event in events[:30]:  # Limit to 30 events per page
        if y < 100:
            p.showPage()
            y = height - 50
        
        text = f"{event.title} - {event.department} - {event.total_registrations} registrations"
